...
Singleton {
    ...
    property bool isMacqueen: false
    ...
    function getScreenScale(screen) {
        ...
        if (isMacqueen && screen) {
            const output = Macqueen.outputs.find(candidate => candidate.name === screen.name);
    ...
    function getFocusedScreen() {
        ...
        else if (isMacqueen && Macqueen.outputs.length > 0)
            screenName = Macqueen.outputs[0].name;
